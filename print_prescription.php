...
echo "Medical Receipt";
echo '<br>';
echo '------------------------------------------------------------------';
$id =  $_POST['id'];

$query = 'SELECT id,name,age,gender,email,phone_no FROM patients WHERE id like "'.$id.'" ';
...
$query = 'SELECT * FROM medicines_selected WHERE id like "%'.$id.'%" ';
...
if(mysqli_num_rows($result) > 0){
	...
	echo $link;
	echo '<br>';
	echo 'No. of Tablets: ', $count;
	echo 'Tablets<br>';
	echo $morning,' ', $afternoon,' ', $night;
	echo '<br>';
        echo $food;
	echo '<br>';
	echo '------------------------------------------------------------------';
...
echo '<br><br><br><button onclick="window.print()">Print this page</button>';
echo $html;
exit;
?>
</div>
